...
![Extendible Hash Table](../graphs/hashes/extendible_put_batched_cpu.png)

Все время тратится только на операции с буфером.
...
На графике ниже результаты бенчмарка, где каждый раз используется ключ из пула в 500, что подтверждает теорию

![](../graphs/hashes/extendible_get_500.png)
...
![](../graphs/hashes/perfect_get_500.png)
...
Операция получения дублей из таблицы LSH в реализации - это получение `List<List<Integer>>`, то есть получение всех существующих бакетов и идентификаторов векторов в них, а после внутри каждого из бакетов в два цикла проверяем все пары векторов на равенство.

![](../graphs/hashes/lsh_doubles.png)

Работает за квадратичное время от количества записей, как и ожидалось.

Профилирование CPU получения:

![](../graphs/hashes/lsh_doubles_prof.png)

Все время занимает итерирование и создание структур, которые мы хотим вернуть

![](../graphs/hashes/lsh_doubles_mem.png)
